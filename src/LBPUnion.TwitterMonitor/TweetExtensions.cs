...
public static class TweetExtensions
{
    public static Embed ToEmbed(this Tweet tweet)
    {
        ...
        if (tweet.Attachments != null && tweet.Attachments.Media != null && tweet.Attachments.Media.Length != 0)
        {
            if (tweet.Attachments.Media[0] != null && !string.IsNullOrWhiteSpace(tweet.Attachments.Media[0].Url))
            {
        ...
        if (tweet.Entities != null)
        {
            ...
            {
                if (!url.DisplayUrl.Contains("pic.twitter.com"))
                {
                    // Replace normal URLS
            ...
            {
                description = description.Replace('#' + hashtag.Tag,
                    $"[#{hashtag.Tag}](https://twitter.com/hashtag/{hashtag.Tag}?src=hashtag_click)");
            }
